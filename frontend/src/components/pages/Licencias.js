...
import { FaInfoCircle } from "react-icons/fa"; // icono info
    ...
    return (
        <div>
            ...
            >
                ...
                <p>
                    {t("Este proyecto es parte de un ")}<strong>{t("proyecto final educativo")}</strong>{t(" y está destinado a demostrar la capacidad técnica de los estudiantes que lo desarrollan. El pago simulado que aparece en esta plataforma es completamente ")}<strong>{t("ficticio")}</strong>{t(" y solo se utiliza con fines educativos para ilustrar el proceso de integración de ")}<strong>{t("Stripe")}</strong>{t(". ")}<strong>{t("No se realizará ninguna transacción real, y no se almacenará dinero ni información financiera")}</strong>{t(". Es importante que sepas que la información ingresada, aunque se procese a través de un sistema simulado, no será utilizada para ningún otro fin fuera del ámbito de esta prueba educativa.")}
                </p>
            ...
            >
                ...
                <p>
                    {t("Al utilizar este sitio, estás aceptando que ")}<strong>{t("el proceso de pago es simulado y no tiene ningún valor real")}</strong>{t(". La plataforma utiliza ")}<strong>{t("Stripe")}</strong>{t(" para simular la realización de pagos y no hay ninguna transacción de dinero real. Las interacciones con la plataforma deben entenderse dentro del contexto de un entorno de aprendizaje.")}
                </p>
                <p>
